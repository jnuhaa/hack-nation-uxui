...
Repo-owned artwork lives under **`media/`** (not generated by MCP). Use these paths when updating the site or social mocks instead of ad hoc URLs.

| Path | Use |
| --- | --- |
| **`media/backgrounds/`** | Raster backgrounds (e.g. **`hero-bg.png`**). Reference for spectral palette / QA and optional fallbacks; the hero may still be shader-driven in production. |
| **`media/community/`** | **Community photography** — avatars, scrolling people strips, and social crops. Add new JPG/PNG exports here and reference relative paths in HTML/CSS. |
| **`media/icon-brandmark.svg`** | **Website navbar** — compact mark only. In markup, use **class `icon-brandmark`** inside `.nav-brand` (see `index.html`). |
| **`media/icon-full.svg`** | **Social posts** — full wordmark for **`Promo_Top`**. Place **`icon-full`** at the **top-left** of the post canvas (Figma **`Vector`** slot, ~**54×54–127×33** bounds depending on template), before the text chips. On dark story-style backgrounds, apply **`icon-full--invert`** so the mark stays legible. |
...
- **Navigation**: centered floating bar with **`media/icon-brandmark.svg`** (**class `icon-brandmark`**) + section anchors + Apply — see **Website** typography for chrome colors and blend behavior
...
- **`Promo_Top`** — **`media/icon-full.svg`** (**`icon-full`**) at **top-left**, then three labels: “Hack-Nation”, “Global AI hackathon”, “In collaboration with MIT Sloan Club” (Figma `Vector` ~54×58 slot maps to this asset + chips).
...
8. **Files**: New backgrounds → **`media/backgrounds/`**; new people shots → **`media/community/`**; social header logo → **`media/icon-full.svg`** (**`icon-full`**, top-left); navbar / compact UI → **`media/icon-brandmark.svg`** (**`icon-brandmark`**).
